...
const PlayMemoryPage = () => {
  ...
  useEffect(() => {
    if (!id) return;
    getMemoryById(id).then((data) => setMemory(data));
  }, [id]);
  ...
  const handleEncodedSong = async (memoryId: string, isVoice: boolean) => {
    try {
      ...
      const fullText =
        `feeling: ${decoded.emotion}, ` +
        `season: ${decoded.season} ` +
        `${decoded.year}, ` +
        `memory: ${decoded.memory_fragment}`;
  ...
  return (
    <>
      <div className={styles['your-memory-container']}>
        ...
        {encodedSongUrl && (
          <div
            className={`${styles['audio-player-container']} ${isEncoding ? styles['hidden'] : ''}`}
        ...
        {!isEncoding && !voiceLayer && encodedSongUrl && (
