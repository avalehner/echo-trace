...
export const getMemories = async (filters?: MemoryFiltersTypes): Promise<MemoryTypes[]> => {
  ...
  if (!response.ok)
    throw new Error(`Server error (getMemories memoriesService.ts): ${response.status}`);
...
export const getMemoryById = async (id: string): Promise<MemoryTypes> => {
  ...
  if (!response.ok)
    throw new Error(`Server error (getMemoryById memoriesService.ts): ${response.status}`);
...
export const createMemory = async (data: NewMemoryTypes): Promise<MemoryTypes> => {
  ...
  if (!response.ok)
    throw new Error(`Server error (createMemory memoriesService.ts): ${response.status}`);
...
export const deleteMemory = async (id: string): Promise<MemoryTypes> => {
  ...
  if (!response.ok)
    throw new Error(`Server error (deleteMemory memoriesService.ts): ${response.status}`);
...
export const generateEncodedSong = async (id: string, isVoice: boolean): Promise<string> => {
  ...
  if (!response.ok)
    throw new Error(`Server error (generateEncodedSong memoriesService.ts): ${response.status}`);
...
export const decodeSong = async (id: string): Promise<DecodedMemoryTypes> => {
  ...
  if (!response.ok)
    throw new Error(`Server error (decodeSong memoriesService.ts): ${response.status}`);
