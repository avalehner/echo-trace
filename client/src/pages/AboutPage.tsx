...
const AboutPage = () => {
  return (
    <div className={styles['about-page-container']}>
      ...
      <p>
        When I'm listening to a song I'm often mentally transported back to the time/or memory that
        I most heavily associate with that song, even if that memory or emotion was not readily
        accessible to me prior to listening to the song. I wanted to create a tool that captures
        this essential part of the human experience, and allows users to actually embed their
        memories into their favorite songs to simulate the effect that listening to music has on the
        human brain.
      </p>
      ...
      <p>
        ...
        that it is invisible to the user. When I first implemented the encoding logic for EchoTrace
        I encoded the users memory data into the least significant bit of every byte of the .wav
        file (or however many bytes necessary to completely encode the memory string). This
        completely concealed the message to the user, so when listening to their 'encoded' .wav
        file, there was no audible difference between the original .wav file--the song sounded
        exactly the same.{' '}
      </p>
      <p>
        Even though the encoding was working, it was an unsatisfying user experience because there
        was no way to verify that the app had actually been encoded in the .wav file. So, I decided
        to 'break' the steganography and started encoding the users data into the most significant
        bit in every byte (the first bit). This produces distortion to the encoded portions of audio
        that results in a buzzing sound, giving the user some indication that their songs .wav file
        had actually been manipulated to include their memory data.
      </p>
    </div>
  );
};
