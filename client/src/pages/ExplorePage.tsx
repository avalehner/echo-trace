...
const ExplorePage = () => {
  ...
  const renderMemories = () => {
    return memories.map((memory) => {
      return <MemoryLog key={memory.id} memory={memory} />;
    });
  };
      ...
      <div className={styles['explore-page-container']}>
        <div className={styles['all-filters-container']}>
          <div className={styles['emotion-menu-container']}>
            ...
            <button className={styles['reset-btn']} onClick={() => handleFilterReset('emotion')}>
              RESET
            </button>
          </div>
          <div className={styles['season-menu-container']}>
            <SeasonMenu season={season} setSeason={setSeason} />
            <button className={styles['reset-btn']} onClick={() => handleFilterReset('season')}>
              RESET
            </button>
          </div>
          <div className={styles['year-menu-container']}>
            <YearMenu year={year} setYear={setYear} />
            <button className={styles['reset-btn']} onClick={() => handleFilterReset('year')}>
              RESET
            </button>
          </div>
        </div>
        <div className={styles['memories-container']}>
          {memories.length === 0 ? (
            <p className={styles['no-memories']}>no memories found </p>
          ) : (
            renderMemories()
          )}
        </div>
